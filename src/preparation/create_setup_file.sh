...
if [[ ${main_dir} == *"src"* ]]; then
  site_params="${data_dir}${sl}input${sl}site_params.csv"

else
  site_params="${main_dir}${sl}input${sl}site_params.csv"
fi
...
OPTIONS:
   ...
   -L      frequency days at which hydraulic legacies are accounted for, e.g.
           arg=20 computes hydraulic legacies every 20 days
   -O      frequency days at which the optimal leaf N allocation module is
           called, e.g. arg=14 calls it every 14 days
   -t      run name tag, e.g. arg=test. The default tag is based on above -m arg
   -w      soil hydrology, either dynamic (default), or arg=prescribed
   ...
   -o      name of the run file created, e.g. runfile.txt
EOF
...
if [[ -z ${fhydralegs+x} ]]; then # default to ignoring legacies
  fhydralegs="None"
fi

if [[ -z ${fleafNopt+x} ]]; then # default to no leaf N optimisation
  fleafNopt="None"
fi
...
if [[ -z ${sw+x} ]]; then # default is to run the dynamic hydrology
  sw="default"
fi
...
# configurations are run (i.e., both 'Control' and 'ProfitMax').
# Possible choices are: 'Control', 'ProfitMax'; any of those two configurations
# can be run alone.
...
while getopts "hs:y:d:N:p:l:m:r:L:O:t:w:P:FR:o:" OPTION; do
