...
def read_netcdf(fname, var_list=None):
    ...
    ds = xr.open_dataset(fname)  # access the data
    dates = pd.to_datetime(ds.time.values)  # retrieve dates

    # drop grid and soil depth
    if var_list is None:
        df = ds.squeeze(dim=('x', 'y', 'z'), drop=True).to_dataframe()

    else:
        df = (ds[var_list].squeeze(dim=('x', 'y', 'z'), drop=True)
                          .to_dataframe())
