...
"""
Functions related to thermodynamics and atmospheric processes relevant
at the canopy level.
...
* Wang, J., Endreny, T. A., & Nowak, D. J. (2008). Mechanistic
  simulation of tree effects in an urban water balance model 1. JAWRA
  Journal of the American Water Resources Association, 44(1), 75-85.

"""

__title__ = "Canopy atmospheric processes"
__author__ = "Manon E. B. Sabot"
__version__ = "2.0 (13.10.2021)"
...
def canopy_intercept(p):

    """
    Throughfall and evaporation at the potential/equilibrium rate,
    where aerodynamic conductance is zero (i.e. winds are calm).
    ...
    Returns:
    --------
    The canopy throughfall [mm d-1] and evaporation at the leaf
    surface [mmol m-2 s-1], using Penman Monteith.
    ...
    Rnet = net_radiation(p, surface='veg')  # W m-2

    # interception coefficient, Wang et al. (2008)
    ci = np.maximum(0., 1. - np.exp(-p.kcanint * (p.LAI + 0.8)))

    # throughfall and evap from canopy interception
    throughfall = (1. - ci) * p.precip  # mm d-1

    # evap from the interception
    gH, __, __ = conductances(p)
    Eci = np.maximum(0., (ci * (Rnet * slp + p.VPD * gH * cst.Cp) /
                          ((slp + gamm) * Lambda)))

    return throughfall, Eci * conv.MILI
