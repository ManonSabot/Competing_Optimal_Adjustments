...
def f(P, b, c, recovery=1., legacy=0.):
    ...
    recovery: float
        dimensionless ratio of xylem recovery from embolism

    legacy: float
        vulnerability associated with the lowest water potential that
        was experienced over a chosen timeframe
    ...
    VC = np.exp(-(-P / b) ** c)
    VC = np.minimum(VC, recovery * (VC - legacy) + legacy)

    return np.maximum(cst.zero, VC)


def transpiration(P, kmax, b, c, recovery=1., legacy=0.):
    ...
    for i in range(len(P)):  # at Ps, trans=0; at Pcrit, trans=transcrit
        ...
        trans[i], __ = quad(f, P[i], P[0], args=(b, c, recovery, legacy))
...
def hydraulics(p, res='low'):
    ...
    trans = transpiration(P, p.kmax, b, c, recovery=p.recovembo,
                          legacy=p.legembo)  # mol m-2 s-1
